...
actual suspend fun pickImportSpreadsheet(): ImportSheet? {
    val (fileName, rawText, error) = nativePickImportTextFile()
    if (fileName == null) return null
    if (rawText == null) {
        return ImportSheet(
            fileName = fileName,
            sheetName = "iOS Files",
            platformNotice = error ?: "This file format is not yet parsed natively on iOS.",
        )
    }
    val records = parseCsvRecordsIos(rawText).filter { row -> row.any { it.isNotBlank() } }
    if (records.isEmpty()) {
        return ImportSheet(fileName, "CSV", rawCsv = rawText, platformNotice = "The selected CSV is empty.")
    }
    val headers = records.first().map(::normalizeHeaderIos)
...
private fun parseCsvRecordsIos(text: String): List<List<String>> {
...
private fun normalizeHeaderIos(value: String) = value.trim().lowercase()
